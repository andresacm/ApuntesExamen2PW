...
const Docentes = () => {
    ...
    return (
        <div>
            ...
            <div>
                <Alert isVisible={alertData.isVisible} message={alertData.message} variant={alertData.variant} setIsVisible={handleOcultarAlerta}/>
            </div>
            <div>
                    ...
                    <tbody>
                        { docentes.map(item => (
                            <tr>
                                ...
                                <td>{item.lastname}</td>
                                <td>
                                    <button onClick={() => handleModificar(item)}>Modificar</button> 
            ...
            <div>
                <Button variant="primary" 
                    style={{ marginBottom: '10px', marginLeft: '10px'}} onClick={() => handleAgregarDocente()}>+ Agregar nuevo docente</Button>
            </div>
                {isFormVisible && <FormularioDocente onClick={handleGuardarDocente} docente={docente} onCancelar={handleCancelarFormulario}/>}
